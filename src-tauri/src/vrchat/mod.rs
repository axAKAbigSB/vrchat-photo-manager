...
#[derive(Deserialize)]
pub struct FileVersion { pub version: i64, pub file: FileBlob }
#[derive(Deserialize)]
pub struct FileBlob { pub url: String }
...
fn session(conn: &Connection) -> Result<String> {
    conn.query_row("SELECT value FROM settings WHERE key='vrchat_session'", [], |r| r.get(0))
        .context("尚未登录 VRChat。请先在设置中登录。")
}

pub async fn login(conn: &Connection, username: &str, password: &str) -> Result<String> {
    let response = Client::builder().user_agent("VRC-Album/0.1 (personal photo manager)").build()?
        .get(format!("{BASE}/auth/user")).basic_auth(username, Some(password)).send().await?;
    if response.status() == reqwest::StatusCode::UNAUTHORIZED { bail!("账号、密码或 2FA 验证失败。请在设置中完成 2FA。") }
    let cookies: Vec<String> = response.headers().get_all(header::SET_COOKIE).iter()
        .filter_map(|value| value.to_str().ok()).filter_map(|value| value.split(';').next()).map(ToOwned::to_owned).collect();
    if cookies.is_empty() { bail!("VRChat 未返回登录会话。") }
...
pub async fn user(conn: &Connection, user_id: &str) -> Result<User> {
    let response = client(&session(conn)?)?.get(format!("{BASE}/users/{user_id}")).send().await?.error_for_status()?;
    Ok(response.json().await?)
}

async fn current_user(conn: &Connection) -> Result<User> {
    let response = client(&session(conn)?)?.get(format!("{BASE}/auth/user")).send().await?.error_for_status()?;
    Ok(response.json().await?)
}
...
pub async fn sync_own_gallery(conn: &Connection) -> Result<usize> {
    let response = client(&session(conn)?)?.get(format!("{BASE}/files?tag=gallery&n=100")).send().await?.error_for_status()?;
    let files: Vec<File> = response.json().await?;
    let own = current_user(conn).await?;
    let mut count = 0;
    for file in files {
        let Some(version) = file.versions.last() else { continue };
        if !file.mime_type.as_deref().is_some_and(|mime| mime.starts_with("image/")) { continue }
        conn.execute(
