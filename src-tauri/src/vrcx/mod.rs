...
fn vrcx_database() -> Option<PathBuf> {
    ...
    if let Ok(text) = std::fs::read_to_string(config) {
        if let Ok(value) = serde_json::from_str::<serde_json::Value>(&text) {
            if let Some(path) = value.get("VRCX_DatabaseLocation").and_then(|v| v.as_str()).filter(|p| !p.is_empty()) {
...
pub fn import(app: &Connection) -> Result<usize> {
    let Some(path) = vrcx_database() else { return Ok(0) };
    ...
    let mut count = 0;
    let mut rows = source.prepare(&format!("SELECT user_id,display_name,trust_level FROM {table}"))?
    ...
    let exists: bool = source.query_row("SELECT EXISTS(SELECT 1 FROM sqlite_master WHERE type='table' AND name=?1)", [history.as_str()], |r| r.get(0))?;
    if exists {
        let sql = format!("SELECT user_id,display_name,previous_display_name,created_at FROM {history} WHERE display_name IS NOT NULL");
        let mut history_rows = source.prepare(&sql)?.query_map([], |r| Ok((r.get::<_, String>(0)?, r.get::<_, String>(1)?, r.get::<_, Option<String>>(2)?, r.get::<_, String>(3)?)))?;
...
pub fn ensure_available() -> Result<()> {
    if vrcx_database().is_none() { bail!("找不到 VRCX.sqlite3；请在设置中配置 VRCX 数据库路径。") }
    Ok(())
}
